...
Your extension runs in Fusion only after it is **built**, **deployed** to Adobe, and (for general availability) **approved** for your organization. This page walks through each step and how to verify the result.

It mirrors Adobe's official [UI Extension development flow](https://developer.adobe.com/uix/docs/guides/development-flow/) and [UI Extensions Management](https://developer.adobe.com/uix/docs/guides/publication/), with the Fusion-specific details filled in.
...
```
  ...
  build â†’ deploy to Stage                build â†’ deploy to Production
        â”‚                                      â”‚
        â–¼                                      â–¼
...
> **Roles:** creating and deploying needs the **Developer** role; submitting the approval request to publish needs a **System Administrator** role. See [setup](./02-setup.md) and Adobe's [How to Get Access](https://developer.adobe.com/uix/docs/guides/get-access/).

---

## Part A â€” Test on Stage

### Step 1 â€” Select the Stage workspace
...
2. Open the section that matches your extension point:
   - `fusion/nav-organization/1` â†’ the **Organization** area of the left navigation.
   - `fusion/nav-team/1` â†’ the **Team** area (select a team first).
...
## Part B â€” Release on Production

When your extension works on Stage and you are ready for all users.

### Step 1 â€” Switch to the Production workspace
...
### Step 2 â€” Build and deploy to Production
...
### Step 3 â€” Submit the approval request
...
To ship changes, repeat **build â†’ deploy** in the appropriate workspace (and, for Production, re-submit for approval if required). Users pick up the new version the next time they open your extension; there is nothing for them to install.
