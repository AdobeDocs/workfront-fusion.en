...
* Opens the new scenario in a separate browser window.
* Replaces the selected modules in the original scenario with a Chain > Call a child scenario module.
...
* Automatically updates mappings:
  * Modules moved into the child scenario receive data through the Chain > Receive data from parent module's inputs.
...
* A selected module has an error handler route, and you have not also selected that route.
...
1. Select the modules that you want to move into a chain by holding [!UICONTROL Shift] and clicking on the modules that you want to move.
